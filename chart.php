...
while ($row = $result->fetch_assoc()) {
    array_push($dataPoints1, array("x" => intval($row['bkbScore']), "y" => intval($row['PhonemesCorrect'])));
    array_push($dataPoints2, array("x" => intval($row['bkbScore']), "y" => intval($row['ThreeWordsCorrect'])));
    //$dataPoints1['array']+=array("x"=>$row['azbioScore'], "y"=>$row['Phonemes Correct']);
    //$dataPoints2['array']+=array("x"=>$row['azbioScore'], "y"=>$row['Words with 3 Phonemes Correct']);
}
...
<div><?php foreach ($dataPoints1 as $key=>$value){
